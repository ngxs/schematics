...
describe('NGXS Store', () => {
  ...
  it('should manage name only', () => {
    ...
    expect(files).toEqual(['/src/todos/todos.actions.ts', '/src/todos/todos.state.ts']);
  });
  ...
  it('should manage name with spec', () => {
    ...
    const files: string[] = tree.files;
    expect(files).toEqual(['/src/auth/auth.actions.ts', '/src/auth/auth.state.spec.ts', '/src/auth/auth.state.ts']);
  });
});
